...
registerCapabilityHandler(executeOpportunitySearchCapability, async ({ ctx, input, idempotencyKey }) => {
  ...
  await supabase.from("opportunity_searches").update({ status: "running", approved_by: ctx.userId, approved_at: startedAt, last_run_at: startedAt, last_run_status: "running", updated_at: startedAt }).eq("workspace_id", ctx.workspaceId).eq("id", search.id);
  ...
  for (const candidate of executed.candidates) {
    ...
    const { data: evidence, error: evidenceError } = await supabase.from("evidence_records").insert({
      ...
      summary: `${candidate.title} returned by the approved ${candidate.sourceSlug} adapter.`,
    ...
    const values = {
      ...
      relationship_score: bestMatch ? Math.round(bestMatch.score * 100) : candidate.relationshipScore,
    ...
    const { error: observationError } = await supabase.from("opportunity_source_observations").upsert({
      workspace_id: ctx.workspaceId,
      ...
      source_policy_disposition: candidate.sourcePolicyDisposition,
    }, { onConflict: "workspace_id,source_type,external_id,opportunity_id" });
  ...
  await supabase.from("opportunity_search_runs").update({ status, source_reports: executed.reports, result_count: executed.candidates.length, matched_count: matchedCount, completed_at: completedAt, error_summary: failedSources ? `${failedSources} source adapters failed.` : null }).eq("workspace_id", ctx.workspaceId).eq("id", run.id);
  await supabase.from("opportunity_searches").update({ status: "completed", last_run_at: completedAt, last_run_status: status, last_run_summary: summary, updated_at: completedAt }).eq("workspace_id", ctx.workspaceId).eq("id", search.id);
