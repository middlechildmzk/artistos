  ...
  return {
    async authorize(ctx, capabilityName) {
      ...
      if (ROLE_RANK[ctx.role] < ROLE_RANK[capability.scope.minRole]) {
        return { allowed: false, reason: `${capability.scope.minRole} role required`, policy: "authorization.workspace_role" };
      }
      if (capability.scope.resource === "artist" && !ctx.artistId) {
        return { allowed: false, reason: "Artist scope is required", policy: "authorization.artist_scope" };
      }
      return { allowed: true };
    },
    ...
    async createApproval({ ctx, capabilityName, version, input, previewHash }) {
      ...
      const { data, error } = await supabase.from("capability_approvals").insert({
    ...
    async recordAudit(args) {
      const supabase = await createSupabaseServerClient();
      const { data, error } = await supabase.from("capability_audit_log").insert({
