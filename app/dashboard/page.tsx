...
function daysUntil(value: string | null | undefined) {
  ...
  return Math.ceil((releaseDate.getTime() - today.getTime()) / 86_400_000);
}
...
export default async function DashboardPage() {
  ...
  const [workspaceResult, releaseResult, tasksResult, followUpsResult, peopleCountResult, fansCountResult] = await Promise.all([
    ...
    <main className="shell">
      <header className="topbar">
        <div className="brand">
          ...
          <div>
            <div className="eyebrow">{workspace?.name ?? "ArtistOS workspace"}</div>
      ...
      <section className="card release-card" style={{ marginBottom: 16 }}>
        ...
        <h1>{release?.title ?? "Create your first release"}</h1>
        <p className="muted">
      ...
      <section className="grid stats" style={{ marginBottom: 16 }}>
        ...
        <div className="card"><div className="eyebrow">Completed</div><div className="stat-value">{doneTasks.length}</div></div>
        <div className="card"><div className="eyebrow">Industry contacts</div><div className="stat-value">{peopleCountResult.count ?? 0}</div></div>
        <div className="card"><div className="eyebrow">Contactable fans</div><div className="stat-value">{fansCountResult.count ?? 0}</div></div>
      </section>
            ...
            {actionableTasks.length ? actionableTasks.map((task) => (
              <div className="row" key={task.id}>
                <div>
                  ...
                  {task.detail ? <p className="muted" style={{ margin: "5px 0 0" }}>{task.detail}</p> : null}
                </div>
                <form action={toggleTask}>
                  <input type="hidden" name="taskId" value={task.id} />
                  <input type="hidden" name="currentStatus" value={task.status ?? "open"} />
                  <button className="button" type="submit">Complete</button>
                </form>
              </div>
            )) : <div className="empty">No actionable tasks. Add the next release milestone or review completed work.</div>}
          </div>

          {blockedTasks.length ? <div className="card">
            <h2>Blocked</h2>
            {blockedTasks.map((task) => <div className="row" key={task.id}><div><strong>{task.title}</strong><p className="muted" style={{ margin: "5px 0 0" }}>{task.blocked_by}</p></div><span className="pill blocked">Blocked</span></div>)}
          </div> : null}
        </div>
        ...
        <aside className="stack">
          <div className="card">
            ...
            {followUps.length ? followUps.map((interaction) => (
              <div className="row" key={interaction.id}>
                <div><strong>{interaction.subject || interaction.channel || "Outreach"}</strong><p className="muted" style={{ margin: "5px 0 0" }}>{formatDate(interaction.follow_up_due)}</p></div>
                <form action={completeFollowUp}><input type="hidden" name="interactionId" value={interaction.id} /><button className="button" type="submit">Done</button></form>
              </div>
            )) : <div className="empty">No overdue follow-ups.</div>}
          </div>
          <div className="card">
            <h2>Workspace health</h2>
            <div className="row"><span>Membership</span><span className="pill">{membership.role}</span></div>
            <div className="row"><span>Release tasks</span><span>{doneTasks.length}/{tasks.length}</span></div>
            <div className="row"><span>Blocked items</span><span>{blockedTasks.length}</span></div>
          </div>
        </aside>
      </section>
    </main>
