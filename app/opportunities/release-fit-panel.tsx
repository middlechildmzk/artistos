...
function fitLabel(item: ReleaseFitItem) {
  return item.overall === null ? "Match unknown" : `Match ${item.overall}%`;
}

function coverageLabel(item: ReleaseFitItem) {
  return `${item.knownDimensionCount} match signal${item.knownDimensionCount === 1 ? "" : "s"}`;
}

function humanize(value: string | null | undefined, fallback = "Not listed") {
  if (!value) return fallback;
  const label = value.replaceAll("_", " ").replace(/\s+/g, " ").trim();
  return label.charAt(0).toUpperCase() + label.slice(1);
}

function routeStateLabel(value: string) {
  if (value === "route_available") return "Submission route";
  if (value === "route_unverified") return "Confirm route";
  if (value === "route_unknown") return "Route not listed";
    ...
    <section className="rfs-shortlist" aria-label="Release shortlist">
      ...
      {shortlist.length ? <ul className="rfs-shortlist-list">{shortlist.map((item) => (
        <li key={item.opportunityId} className="rfs-shortlist-item">
          <div><strong>{item.title}</strong><span className={`rfs-pill rfs-readiness-${item.readinessState ?? "needs_review"}`}>{(item.readinessState ?? "needs review").replace(/_/g, " ")}</span>{item.blockingReasons.length ? <p className="rfs-blocking">Before submitting: {item.blockingReasons.join(", ").replace(/_/g, " ")}</p> : null}</div>
      ...
      <p className="rfs-note">Saving to your shortlist does not submit or charge anything.</p>
    </section>
  ) : null;
  ...
  return (
    <section className="rfs-root">
        ...
        <div className="rfs-context-actions">
          <form method="get" action="/opportunities" className="rfs-release-switcher">
            <label>
              ...
              <select name="releaseId" defaultValue={release.releaseId} onChange={(event) => event.currentTarget.form?.requestSubmit()}>
      ...
      <details className="rfs-profile-editor">
        <summary><span>Improve release matching</span><small>{release.missingMetadata.length ? `${release.missingMetadata.length} release detail${release.missingMetadata.length === 1 ? "" : "s"} missing` : "Release profile complete"}</small></summary>
      ...
      <div className="rfs-modes" role="tablist" aria-label="Directory view">
        <button type="button" role="tab" aria-selected={mode === "advanced"} className={`rfs-mode ${mode === "advanced" ? "rfs-mode-on" : ""}`} onClick={() => setMode("advanced")}>All opportunities</button>
        <button type="button" role="tab" aria-selected={mode === "recommended"} className={`rfs-mode ${mode === "recommended" ? "rfs-mode-on" : ""}`} onClick={() => setMode("recommended")}>Best matches for {release.title}</button>
      </div>
      ...
      {mode === "advanced" ? <>
        ...
        <>
          <div className="rfs-filter-grid rfs-recommendation-filters">
            ...
            <label className="rfs-check"><input type="checkbox" checked={hideIneligible} onChange={() => setHideIneligible((value) => !value)} /><span>Hide targets that do not accept this release</span></label>
            <label className="rfs-check"><input type="checkbox" checked={onlyEvidenced} onChange={() => setOnlyEvidenced((value) => !value)} /><span>Only show strong match evidence</span></label>
          </div>

          {shortlistPanel}

          <p className="rfs-count">{visible.length} recommended {visible.length === 1 ? "opportunity" : "opportunities"}</p>
          <ul className="rfs-results">
            {visible.map((item) => (
              <li key={item.opportunityId} className="rfs-card">
                <div className="rfs-card-head"><div className="rfs-card-identity"><strong className="rfs-card-title">{item.title}</strong><span className="rfs-card-sub">{[humanize(item.targetType, ""), item.country].filter(Boolean).join(" · ") || "Opportunity"}</span></div><div className="rfs-card-score"><span className={`rfs-fit ${item.overall === null ? "rfs-fit-unknown" : ""}`}>{fitLabel(item)}</span><span className="rfs-coverage">{coverageLabel(item)}</span></div></div>
                <div className="rfs-tags">{item.platforms.map((platform) => <span key={platform} className="rfs-pill">{platform}</span>)}{item.genres.slice(0, 3).map((genre) => <span key={genre} className="rfs-pill">{genre}</span>)}<span className="rfs-pill">{item.activityLabel ?? "Activity unknown"}</span><span className={`rfs-pill rfs-route-${item.routeState}`}>{routeStateLabel(item.routeState)}</span><span className="rfs-pill">{item.audienceLabel}</span></div>
                {item.explanations.length ? <ul className="rfs-why">{item.explanations.slice(0, 3).map((line) => <li key={line}>{line}</li>)}</ul> : <p className="rfs-why-none">No clear match evidence yet.</p>}
                <div className="rfs-card-actions">
                  ...
                  <button type="button" className="rfs-btn" onClick={() => setOpenId(item.opportunityId)} aria-haspopup="dialog">Why it matches</button>
                </div>
              </li>
            ))}
          </ul>
      ...
      {openItem ? (
        <div className="rfs-slideover-layer" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpenId(null); }}>
          <aside className="rfs-slideover" role="dialog" aria-modal="true" aria-label={`Why ${openItem.title} matches`}>
            ...
            <section><h4>Submission details</h4><ul className="rfs-policy"><li>Route: {routeStateLabel(openItem.routeState)}</li><li>Cost: {openItem.routeIsFree === null ? "Not listed" : openItem.routeIsFree ? "Free" : "Paid"}</li><li>AI policy: {humanize(openItem.aiPolicy)}</li><li>References checked: {openItem.corroborationCount}</li><li>Last checked: {humanize(openItem.sourceFreshness)}</li></ul><p className="rfs-note">Confirm the current route and terms before submitting.</p></section>
          </aside>
        </div>
      ) : null}
    </section>
