...
create policy "profiles owner all" on public.artist_platform_profiles for all to authenticated using (owner_id = auth.uid()) with check (owner_id = auth.uid());
create policy "release links owner all" on public.release_platform_links for all to authenticated using (owner_id = auth.uid()) with check (owner_id = auth.uid());
create policy "metric snapshots owner all" on public.music_metric_snapshots for all to authenticated using (owner_id = auth.uid()) with check (owner_id = auth.uid());
create policy "coverage owner all" on public.music_coverage_events for all to authenticated using (owner_id = auth.uid()) with check (owner_id = auth.uid());
create policy "placements owner all" on public.playlist_placements for all to authenticated using (owner_id = auth.uid()) with check (owner_id = auth.uid());
